...
/**
 * The idea domain, free of any transport. `lib/content/actions.ts` wraps these
 * with `verifySession()` and FormData parsing for the UI; `lib/mcp/tools/ideas.ts`
 * wraps the same functions for MCP clients (issue #109). Validation, release
 * defaults, tag normalisation, revalidation and the Google push-after-commit
 * all live here, so a row written by either door is indistinguishable.
 */
...
export interface IdeaActionState {
  ...
  success?: boolean;
  /** Set on a successful capture — MCP clients need the id to keep working on it. */
  ideaId?: string;
}
...
/** Form-shaped capture input, exactly the fields the capture dialog submits. */
export interface RawIdeaCaptureInput {
  title?: string;
  description?: string;
  format?: string;
  tags?: string;
...
/** Form-shaped edit input — every field except the script (see docs/scripts.md). */
export type RawIdeaEditInput = Omit<RawIdeaCaptureInput, "script">;
 ...
 * `updateIdeaStatusCore` and left untouched here.
 *
...
/** Paths every release write touches — the idea surfaces plus the calendar. */
function revalidateReleasePaths(ideaId: string): void {
  revalidatePath("/content/ideas");
  revalidatePath(`/content/ideas/${ideaId}`);
  revalidatePath("/content/board");
 ...
 * in the shape of `updateIdeaStatusCore`/`rescheduleEventCore`, deliberately not
 * a shortcut into `updateIdeaCore`: shifting a publish slot must not require
 * sending (and re-validating, and re-saving) the title, tags and description the
 * user never opened.
 *
 ...
 * title is left alone; only `updateIdeaCore` owns that, and it's derived from
 * the idea's title, which a reschedule doesn't touch.
 */
export async function rescheduleIdeaReleaseCore(
  ideaId: string,
  releaseDate: string,
  releaseTime: string
  ...
  revalidateReleasePaths(parsed.data.ideaId);
  schedulePush(() => pushEventUpdated(updated[0].id, updated[0].track));
  return {};
}

/**
 * Sets *or* moves an idea's release in one call — the shape an MCP client wants
 * ("push the PoE video to Friday" shouldn't care whether a release exists yet).
 * The UI reaches the same two outcomes through two different surfaces (the edit
 * dialog creates, the release chip moves), so this deliberately delegates to
 * the same release-event helpers rather than re-deriving them.
 *
 * An omitted time defaults to `DEFAULT_RELEASE_TIME` exactly as the capture form
 * does, so a date-only call lands at 19:00 on the calendar (issue #71).
 */
export async function setIdeaReleaseCore(
  ideaId: string,
  releaseDate: string,
  releaseTime?: string
): Promise<RescheduleIdeaReleaseResult> {
  ...
  const release = buildReleaseSpan(parsed.data.releaseDate, time);
  if (!release) {
    return { error: INVALID_RELEASE };
  }
  ...
  const [existing] = await db
    .select({
      title: ideas.title,
      releaseEventId: ideas.releaseEventId,
    })
    .from(ideas)
    .where(eq(ideas.id, parsed.data.ideaId));
  if (!existing) {
  ...
  if (existing.releaseEventId) {
    await updateReleaseEvent(
      db,
      existing.releaseEventId,
      existing.title,
      release
    );
  } else {
    await createReleaseEvent(db, parsed.data.ideaId, existing.title, release);
  }
...
/** Unschedules a release: deletes the managed event, leaving the idea itself intact. Clearing an already-unscheduled idea is an idempotent success. */
export async function clearIdeaReleaseCore(
  ideaId: string
): Promise<RescheduleIdeaReleaseResult> {
  const db = getDb();
  const [existing] = await db
  ...
  if (!existing.releaseEventId) return {};

  await deleteReleaseEvent(db, existing.releaseEventId);
  revalidateReleasePaths(ideaId);
  return {};
}
 ...
 * clock below) stays consistent between them. Mirrors `rescheduleEventCore`'s
 * narrow-mutation shape in `lib/calendar/core.ts`.
 *
 ...
 */
export async function updateIdeaStatusCore(
  id: string,
  status: string
 ...
 */
export async function deleteIdeaCore(id: string): Promise<DeleteIdeaResult> {
  const db = getDb();
  const [existing] = await db
    .select({ releaseEventId: ideas.releaseEventId })
    .from(ideas)

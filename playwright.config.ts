...
export default defineConfig({
  ...
  projects: [
    ...
    },
    {
      ...
      // and mobile.spec.ts (whose journal and weekly-summary cases write real
      // rows) seed/create/clear real rows (calendar-sync.spec.ts's connection
      // rows are also unique-per-track) in the dev database and already cover
      ...
      testIgnore:
        /(calendar|calendar-sync|event-management|drag-reschedule|agenda|board|scripts|content-links|streams|publish-checklist|journal|weekly-summary|weekly-assessment|mobile)\.spec\.ts$/,
    },
  ],
  webServer: [
    {
